...
const CyclInterface = () => {
    ...
    return (
        <>
            <div className={`w-[95%] lg:w-[90%] dh h-[590px] md:h-[800px] lg:h-[700px] xl:h-[800px] rounded-2xl md:rounded-[50px] flex overflow-hidden relative flex-col ${language=="ar"&&'items-end'} justify-end lg:p-[25px] text-neutral-700 lg:text-5xl xl:text-7xl ${language === 'ar' && '  lg:text-6xl xl:text-7xl 2xl:text-8xl text-right'}`}>
                ...
                <div className={`w-fit  hidden lg:block bg-white  -z-10 ${language=="ar"&&"-scale-x-100 flex flex-row-reverse"} rounded-t-[25px] p-[30px] pb-0 flex items-end justify-end   gap-x-2 ${language === 'ar' && 'flex-row-reverse'} psd`}>
                    <div className={`flex justify-center items-center ${language=="ar"&&" -scale-x-100 gap-x-4 flex-row-reverse"}`}>
                ...
                <div className={`w-[45%]  lg:w-[55%] xl:w-[50%] 2xl:w-[45%] ${language==="fr"&&"w-[45%]  lg:w-[50%] xl:w-[58%] 2xl:w-[45%]"}   hidden -z-10 lg:block  bg-white ${language=="ar"?"rounded-l-[25px] pb-5":"rounded-r-[25px] pb-1"}   p-[30px] gap-x-2  `}>
                    <div className={`flex items-end gap-x-2 ${language === 'ar' && 'flex-row-reverse'}`}>
                    ...
                    <p className={`xl:text-lg lg:text-base  ${language==="ar"&&"text-xl"} mt-6`}>{content.texts.description[language]}</p>
                </div>

                <div className={`w-fit  hidden lg:block -z-10 bg-white  ${language=="ar"&&"-scale-x-100 "} rounded-b-[25px] p-[30px] pt-0 shadow-lg psdr block`}>
                    <button className={`bg-red-500 px-5 p-3 ${language=="ar"&&"-scale-x-100 "} text-white rounded-full gap-x-2 font-bold text-base xl:text-xl 2xl:text-xl`}>
                        {content.texts.buttonText[language]} <FontAwesomeIcon className={`mx-2`} icon={faArrowDown} />
                    </button>
                </div>


                <div className={`absolute top-3  lg:top-auto w-full lg:w-auto md:bottom-0 right-0 md:block text-center ${language === 'ar' ? 'left-0 md:right-auto' : 'right-0'}`}>
                    <div className={`md:p-[30px] p-1 flex items-center justify-center text-[14px] sm:text-lg lg:text-base xl:text-xl md:gap-x-2 gap-x-1 relative ${language === 'ar' && 'flex-row-reverse'}`}>
                        ...
                        return (
                            <div key={groupKey} className={`rounded-full md:w-[20%]  border-white/30 border  lg:w-auto relative flex items-center justify-center p-1 sm:p-3 xl:p-4 bg-white/30 md:bg-white/${blurLevel} backdrop-blur-lg shadow-2xl`}>
                ...
                <div className={`w-full h-[100%] bg-gradient-to-t from-white to-white/5 gap-y-3 lg:hidden text-neutral-700 text-xl flex flex-col justify-end p-3 ${language === 'ar' && 'text-right items-end'}`}>
                    <div className={`flex items-center text-4xl sm:text-6xl md:text-7xl gap-x-3  ${language === 'ar' && 'flex-row-reverse'}`}>
                    ...
                    <p className={`text-base md:text-lg ${language==="ar"&&"text-xl"} `}>{content.texts.description[language]}</p>

                    <button className={`bg-red-500 p-3 md:py-5 flex items-center justify-center  ${language === 'ar' && 'flex-row-reverse text-xl'} text-white rounded-full font-bold text-base sm:text-lg  w-full`}>
                ...
                <div className={`absolute w-[40%] xl:w-[35%] hidden  top-[20px] right-[20px] lg:flex flex-col items-end text-xl gap-x-2 ${language==='ar'&&'left-[20px] right-[auto]  -scale-x-100'}`}>
            ...
            <div className="w-[90%] flex items-center justify-center text-center mt-10 lg:mt-20 flex-col">
                <p className="text-neutral-500">{content1.texts.discover[language]}</p>
                <h1 className={`text-blue-500 text-4xl md:text-5xl lg:text-6xl xl:text-7xl 2xl:text-8xl ${language=="ar"&&"text-4xl md:text-5xl lg:text-6xl xl:text-7xl 2xl:text-9xl"}`}><b>{content1.texts.nos_cycles_pedagogiques[language]}</b></h1>
                <p className="text-neutral-500 mt-2 md:mt-6 lg:mt-10 text-[10px] md:text-base lg:text-lg lg:w-[95%] ">
                ...
                <div className={`w-full mt-10 lg:flex gap-x-6 lg:gap-x-2 2xl:gap-x-6 items-stretch ${language==="en"?'xl:h-[780px]':'xl:h-[750px]'} lg:h-[600px] overflow-hidden text-white`}>
                    ...
                    <div className="w-full lg:w-[40%] flex flex-col items-center justify-center gap-y-6 lg:gap-y-2 2xl:gap-y-6">
                        <div className="w-full h-[80%] rounded-[50px] bg-blue-500 flex items-end justify-center overflow-hidden  pt-10">
                        ...
                        <div className={`w-full h-[20%] rounded-[50px] bg-neutral-800 overflow-hidden lg:flex hidden items-center justify-center font-bold px-10 text-left `}>
                    ...
                    <div className="flex-1 rounded-[50px] h-[300px] mt-6 lg:hidden  bg-red-500 overflow-hidden relative">
                    ...
                    <div className="w-full lg:w-[60%] h-full flex-col flex gap-6 lg:gap-2 2xl:gap-6 mt-6 lg:mt-0">
                        ...
                        <div className="w-full lg:h-[45%] h-[200px]  rounded-[50px] bg-red-500 overflow-hidden  flex  relative z-30">
                       ...
                       <div className="flex-1 rounded-[50px] h-full hidden lg:block bg-red-500 overflow-hidden relative">
                       ...
                       <div className=" flex-1 rounded-[50px] h-full  bg-blue-500 overflow-hidden relative">
